    ...
    def _login(self) -> None:
        ...
        try:
            ...
            if "token" in token_data and "expiry" in token_data["token"]:
                self.token_expiry = int(token_data["token"]["expiry"])
                self.session = s
                print(f"信息：成功登录到 WorldQuant BRAIN 平台。会话有效期至: {time.ctime(self.token_expiry)}")
            else:
                print("错误 (SessionManager._login): 登录后无法获取有效的token信息。")
                raise ValueError("登录后未能获取token。")
    ...
    def get_session(self) -> requests.Session:
        ...
        if self.session is None or self.token_expiry == 0 or current_time >= (self.token_expiry - 300):
            if self.session is not None and self.token_expiry > 0 :
                print(f"信息：当前会话即将过期 (剩余 {(self.token_expiry - current_time)//60} 分钟)，正在尝试重新登录...")
            else:
            ...
            try:
                ...
                # 根据实际需求，这里可以决定是重试、抛出更具体的异常，还是返回None/旧session（如果不严格要求最新）
                # 为了保证返回的是有效会话，如果登录失败，则应抛出异常
