...
class BrainApiClient:
    ...
    def _handle_single_alpha_simulation_and_tagging(self, alpha_sim_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'Location' not in response_start_sim.headers:
                ...
                raise ValueError("模拟启动响应缺少Location头部")
    ...
    def run_simulation_workflow(
        ...
        """
        print(f"信息: 开始执行模拟工作流 (深度 {depth}, 迭代 {iteration if iteration > 0 else '初始化'}), 总计 {len(alpha_sim_data_list)} 个Alpha, 并发数 {limit_concurrent}.")

        effective_concurrency = max(1, limit_concurrent)
        final_results_list: List[Dict[str, Any]] = []
        initial_simulation_results: List[Dict[str, Any]] = []

        progress_desc_sim = f"并发模拟Alpha (深度{depth}, {'迭代'+str(iteration) if iteration > 0 else '初始化'})"
        with ThreadPool(processes=effective_concurrency) as pool_sim_start:
            with tqdm.tqdm(total=len(alpha_sim_data_list), desc=progress_desc_sim, unit="alpha") as pbar_sim_start:
                for result in pool_sim_start.imap_unordered(self._handle_single_alpha_simulation_and_tagging, alpha_sim_data_list):
                    initial_simulation_results.append(result)
                    pbar_sim_start.update(1)

        print(f"信息: 所有 {len(alpha_sim_data_list)} 个Alpha的模拟启动（和标签设置）已完成。")

        valid_initial_results = [res for res in initial_simulation_results if res.get('alpha_id') is not None]
        failed_to_get_id_results = [res for res in initial_simulation_results if res.get('alpha_id') is None]

        print(f"信息: {len(valid_initial_results)} 个Alpha成功获取ID，将获取其详细统计数据。{len(failed_to_get_id_results)} 个Alpha未能获取ID。")

        if valid_initial_results:
            progress_desc_stats = f"获取Alpha统计 (深度{depth}, {'迭代'+str(iteration) if iteration > 0 else '初始化'})"
            tasks_for_stats = [(res['alpha_id'], res['simulate_data']) for res in valid_initial_results]

            with ThreadPool(processes=effective_concurrency) as pool_stats_get:
                with tqdm.tqdm(total=len(tasks_for_stats), desc=progress_desc_stats, unit="alpha") as pbar_stats_get:
                    def stats_task_wrapper(args_tuple):
                        return self._get_specified_alpha_stats_details(args_tuple[0], args_tuple[1])

                    for detailed_result in pool_stats_get.imap_unordered(stats_task_wrapper, tasks_for_stats):
                        final_results_list.append(detailed_result)
                        pbar_stats_get.update(1)

        for failed_res in failed_to_get_id_results:
            final_results_list.append({
                'alpha_id': None, 'simulate_data': failed_res['simulate_data'],
                'is_stats': pd.DataFrame(), 'pnl': pd.DataFrame(), 'stats': pd.DataFrame(),
                'is_tests': pd.DataFrame(), 'error': "未能成功启动模拟或获取Alpha ID。"
            })

        print(f"信息: 模拟工作流 (深度 {depth}, 迭代 {iteration if iteration > 0 else '初始化'}) 完成。总共处理 {len(final_results_list)} 个结果。")
